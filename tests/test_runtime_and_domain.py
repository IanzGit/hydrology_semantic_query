...
from app.agents.scenarios.cqccri_smart_query.subgraph.hydrology_semantic_query.query_child.tools.run_semantic_query import (
...
def test_all_public_views_accept_basic_semantic_queries() -> None:
    ...
    for view in PUBLIC_VIEWS:
        ...
        validate_semantic_query(query, catalog)
...
def test_catalog_public_model_filter_and_recursive_validation() -> None:
    ...
    validated = validate_semantic_query(query, catalog)
    assert validated.query.limit == 5000
...
def test_top_level_measure_and_dimension_filters_can_coexist() -> None:
    ...
    validated = validate_semantic_query(query, catalog_from_meta(_meta()))
...
def test_filter_operator_compatibility_and_value_counts(filter_payload: dict) -> None:
    ...
    with pytest.raises(SemanticQueryValidationError):
        validate_semantic_query(query, catalog_from_meta(_meta()))
...
async def test_cube_client_closes_method_scoped_clients(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "app.agents.scenarios.cqccri_smart_query.subgraph.hydrology_semantic_query.query_child.client.httpx.AsyncClient",
        ScopedClient,
    )
    client = CubeClient(
        base_url="http://cube.internal/cubejs-api/v1",
