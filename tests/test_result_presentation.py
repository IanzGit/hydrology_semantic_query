...
def test_scalar_result_outputs_kpi_and_detail_table() -> None:
    result = _result(
...
def test_explicit_pie_overrides_default_bar_without_dropping_table() -> None:
    ...
    assert _chart_payload(outputs[0])["chartType"] == "PIE"
...
def test_latest_kpi_compares_timezone_offsets_chronologically() -> None:
    ...
    assert _chart_payload(outputs[0])["seriesData"][0]["data"] == [
        {"name": "水位", "value": 2.0}
    ]
...
def test_status_field_outputs_status_cards_and_table() -> None:
    ...
    assert status_block.data["items"][0] == {
        "label": "严重告警",
        "count": 2,
        "severity": "critical",
        "color": "#d94b4b",
...
def test_alarm_names_remain_categories_and_alarm_levels_render_separate_pies() -> None:
    ...
    assert [chart["data"]["chartName"] for chart in charts] == [
        "当前报警等级分布",
        "历史最高报警等级分布",
    ]
...
def test_coordinates_output_map_and_preserve_structured_points() -> None:
    result = _result(
        [
            _column("station", "站点", "string", "dimension"),
    ...
    assert _chart_payload(outputs[0])["chartType"] == "BAR"
    map_block = next(
        block for block in _blocks(result) if block.type == PresentationBlockType.MAP
    )
    assert map_block.data["points"][0]["name"] == "A"
