...
def _failure_outcome(state: HydrologySemanticQueryState) -> tuple[QueryOutcome, SemanticQueryError | None]:
    failures = [
        item
...
def _task_result_payload(result: TaskExecutionResult) -> dict[str, Any]:
    ...
    return {
        ...
        "rows": record.rows[:50] if record else [],
        "rows_truncated": bool(record and len(record.rows) > 50),
        "error": (
            {
...
def make_query_dispatch_node(
    ...
    async def dispatch(
        ...
        child_metadata = {
            ...
            "max_rows": state.get("max_rows", services.settings.max_rows),
        }
        ...
        record = child_result.query_history[-1] if child_result and child_result.query_history else None
